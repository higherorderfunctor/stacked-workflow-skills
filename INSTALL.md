...
## Nix: Home-Manager Module (Recommended)
...
The flake also provides git config presets for stacked workflows:
...
For consumers who don't use the module (e.g., NixOS system config, custom
devShells), the flake exposes paths directly:

```nix
# lib.paths.skillsDir  — path to skills/
# lib.paths.referencesDir — path to references/

# Example: home.file without the module
...
Then add the routing table to `~/.claude/CLAUDE.md`. Copy the Claude Code
table from the [Routing](#routing) section, or use:
...
2. Choose an integration path:
   - **Home-manager module** — import and enable (see
     [Nix: Home-Manager Module](#nix-home-manager-module-recommended))
